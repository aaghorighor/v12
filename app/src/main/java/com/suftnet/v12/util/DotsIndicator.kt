...
class DotsIndicator(context: Context, attrs: AttributeSet? = null) : LinearLayout(context, attrs) {
    ...
    init {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.TExDotsIndicator)
        ...
        try {
            intrinsicWidth = ta.getDimensionPixelSize(R.styleable.TExDotsIndicator_dot_width, -1)
            intrinsicHeight = ta.getDimensionPixelSize(R.styleable.TExDotsIndicator_dot_height, -1)
            intrinsicMargin = ta.getDimensionPixelSize(R.styleable.TExDotsIndicator_dot_margin, -1)
            intrinsicOrientation = ta.getInt(R.styleable.TExDotsIndicator_dots_orientation, -1)
            intrinsicGravity = ta.getInt(R.styleable.TExDotsIndicator_dots_orientation, -1)

            this.animatorResId = ta.getResourceId(R.styleable.TExDotsIndicator_dots_animator, R.anim.scale_with_alpha)
            this.animatorReverseResId = ta.getResourceId(R.styleable.TExDotsIndicator_dots_animator_reverse, 0)
            this.backgroundResId = ta.getResourceId(R.styleable.TExDotsIndicator_dot_drawable, R.drawable.bg_back_dot)
            this.unselectedBackgroundId = ta.getResourceId(R.styleable.TExDotsIndicator_dot_drawable_unselected, this.backgroundResId)
            this.dotTint = ta.getColor(R.styleable.TExDotsIndicator_dot_tint, 0)
        } finally {
            ta.recycle()
        }
        ...
        indicatorBackgroundResId = if (this.backgroundResId == 0) R.drawable.bg_back_dot else this.backgroundResId
